...
你是一个科研项目管家 (Orchestrator Agent)。你的任务是浏览当前工作目录下的科研项目（当前文件夹下包含项目的idea、plan，前期编写好的代码和运行结果，以及已经写好的论文的tex文件，和审稿人的意见），并根据审稿人的意见修改论文。
当前工作目录下，所有代码都已经根据审稿人的审稿意见更新（包括添加新的baseline，添加导频信号等）。但是论文修改（各个tex文件）还没有完成。
注意，在本轮中，你不需要修改任何python文件，只需要修改论文（tex文件）
你需要做的事情是：
1）确认审稿人的意见
2）通过浏览execute_history.txt，了解之前已经积累的实验数据
3) 通过浏览recorded_data.txt，了解为了回应审稿人的意见，已经产生的新数据
4）根据审稿人的意见和新的数据，修改对应的tex文件。注意，numerical_results的图表在fig1.tex~fig5.tex中，你必须将现有数据和原有数据有机结合（不要增加新的图片，而是要将新获得的数据插入原来的图片中，并改变图片的图例、标题等）
5）一定不能只修改numerical_results.tex，而不修改对应的图片。
6）在修改tex文件时，请不要遗漏原有的内容（除非新增内容和原有内容冲突）。一张图中的曲线越多越好。例如，如果原有曲线不考虑信道估计误差，新增曲线考虑了，则原有曲线可以保留，并放进一张图中。
7）论文中对实现的描述，必须基于对应的代码，如果你在写论文中的某一段时，对具体实现没有把握，应该先阅读对应的代码


PreviousSummary.txt中包含了所有代码的readme文件，曾经AI编写这些代码时所用到的idea和实验计划。
审稿人review之前论文的所有实验数据，在execute_history.txt中记录。
回应审稿人之后产生的新数据，在recorded_data.txt中记录。
如果想要看到某个文件的内容，必须通过`READ_CODE`操作来进行查看。`RUN_CODE`只能用来运行python脚本，你大概率不需要（因为数据已经准备好了）。
...
3. `READ_CODE`: 申请阅读当前代码库中某份代码（或者文件）的全文。
    ...
    while step_idx < 1:
        ...
        while attempts < MAX_RETRIES and not step_passed:
            ...
            if not tool_calls_history:
                ...
                if len(tool_calls_history) > 10:
                    for th in tool_calls_history[-10:]:
...
```
            ...
            if action == "PASS_STEP":
                ...
                if files:
                    ...
                    tool_calls_history.append({"action": action, "params": params, "result": f"Modified article and submitted files: {', '.join(saved_files)}"})
                ...
                with open(data_record_txt, "a", encoding="utf-8") as f:
                    ...
                    tool_calls_history.append({"action": action, "params": params, "result": f"Recorded data: {data_record}"})
            else:
