...
你可以执行的操作（Tools）包括：
1. `SEARCH_LITERATURE`: 查找文献来确认当前的计划具体如何执行。
2. `READ_PAPER`: 申请阅读和当前计划强相关的论文全文（提供 doi 列表）来进一步明确当前计划应该如何执行。
...
8. 原始的idea和plan未必完全合理，当年发现Coder尝试编写了很多次程序之后，仍然达不到想要的结果，可以对idea或者plan本身做适当且合理的修改（体现在你给Coder的instruction中）。
...
```json
{
    ...
    "Action_Params": {
        ...
        "summary": "如果调用 PASS_STEP，在此写当前步骤完成情况的详细总结"
    }
}
...
2. `RUN_CODE`: 编写并运行 bat 脚本以测试你写的代码并观察控制台输出。
...
```json
{
    ...
    "Action_Params": {
        ...
        "run_script": "如果 RUN_CODE，运行用于测试的 bat 脚本内容(必须命名为run.bat)"
    }
}
