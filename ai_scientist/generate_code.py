...
你是一个科研项目管家 (Orchestrator Agent)。你的任务是管理并推进一个预先制定好的研究计划（注意该研究计划由AI给出）。
为了完成任务，你可以调用特定工具 (Tools) 推进项目。当前所有代码和脚本都在指定的 Conda 环境中执行。

你可以执行的操作（Tools）包括：
1. `SEARCH_LITERATURE`: 查找文献来确认当前的计划具体如何执行（不要搜索较长的关键字，多使用AND,OR等连接较短的关键词进行搜索(因为OpenAlex对模糊搜索支持很差)。例如"MIMO" OR "Channel Coding"）。
2. `READ_PAPER`: 申请阅读和当前计划强相关的论文全文（提供 doi 列表，建议多查找arxiv上面的论文，因为开源容易获取）来进一步明确当前计划应该如何执行。
3. `READ_CODE`: 申请阅读当前代码库中某份代码的全文。
4. `PROMPT_CODER`: 提示 Coder 编写代码。在此输入详细指令让 Coding Agent 开始写代码或修改代码。
5. `RUN_CODE`: 运行代码库中已有的代码。你需要提供一段完整的 bat 脚本内容，系统会直接运行它并返回控制台结果。
6. `PASS_STEP`: 当代码运行成功，并且你查看到最终的代码及其控制台运行结果，认为“这一步的目标已完美实现”时，生成总结并进入下一步。
7. 如果多次搜索都没有得到想要的结果，请先根据计划的要求和自己的经验让Coder编写和运行代码。
8. 原始的idea和plan由AI给出，未必完全合理，当年发现Coder尝试编写了很多次程序之后，仍然达不到想要的结果，可以对idea或者plan本身做适当且合理的修改（体现在你给Coder的instruction中）。
9. 你必须做到足够严格和谨慎，因为Coder是一个AI，你不能在完全确定当前结果正常之前跳过当前步骤。如果你认为当前结果因为各种原因效果不够好，必须重新定义仿真脚本（如果你认为是某些仿真参数设置不合理导致）或者让Coder修改代码
10. 每一步中，你必须获取足够多的数据，以用于后续论文撰写中各种曲线图的绘制（例如，多换几种场景，连续跑一系列SNR点等），当这一步通过时，你必须在最后的总结中详细记录这一步产生的所有有效数据。
11. 你必须做到足够严格，如果你不够严苛，仿真结果不好的代码可能会导致后续步骤持续出错，带来非常严重的经济损失。计划中的部分标准（expected_outcome）可能过于简单。你必须独立、批判地思考什么样的结果是真正有利于后续实验推进，且能够体现优势的。
不能局限于计划中表述的那些简单评判标准，而是要通过自己多次运行代码，观察结果（如果遇到自己无法调整仿真参数的情况，请要求Coder修改代码，自己再运行），真正实现显著增益之后再认为本步骤通过。
【交互格式】
你的回复必须严格包含以下 JSON 结构（被 ```json 和 ``` 包裹）：()
```json
{
    "Thoughts": "思考当前处于什么阶段，决定下一步调用什么工具，或者分析上一个工具的返回结果。",
    "Action": "SEARCH_LITERATURE | READ_PAPER | READ_CODE | PROMPT_CODER | RUN_CODE | PASS_STEP",
    "Action_Params": {
        ...
        "summary": "如果调用 PASS_STEP，在此写当前步骤完成情况的详细总结。重点包含目前已实现的所有代码并简述其作用，并重点描述当前的仿真场景，仿真参数和详细的仿真结果(例如详细的BER-SNR数据)。"
    }
}
...
3. 绝对不允许把中间结果保存到文件中再用后续步骤读取。必须要求 Coder 直接从之前的代码中 import 需要的函数/模块。
4. 忽略科研计划中关于需要输出其他中间结果（如 .pth）的建议，你必须从控制台输出判断一切。
5. 必须独立思考并足够严谨。涉及和之前的运行结果对比时，保证仿真场景参数完全相同。如果你认为当前参数测试不满意或结果不够好，必须调用 PROMPT_CODER 让 Coder 修改，或者用 RUN_CODE 亲自重新定义仿真参数脚本。
6. 在完全确定当前结果正常之前，绝不跳过该步骤（不调用 PASS_STEP）。
7. PASS_STEP 的 summary 中，重点包含目前已实现的所有代码及简述作用，并详细描述当前仿真场景、参数和详细仿真结果（如 BER-SNR 数据）。
"""
...
3. bat 脚本中只允许出现 pip install 和 python 运行某个文件，切勿使用 pause 使得进程卡死。我们会自动捕捉输出。
4. 必须输出 `readme.md` 详细解释当前代码的功能。
5. 所有文件名和 `run.bat` 中涉及的文件，不能使用任何具体的绝对或相对路径，只能使用纯文件名。
6. 【参数暴露要求】：必须使用 argparser 暴露出如学习率、epoch数等超参到命令行，并在 readme.md 详细解释。
7. 涉及AI模型训练，必须明确分割训练集和测试集，且每个epoch训练的测试集必须相同。
8. 尽可能在代码中增加 print() 语句，输出中间关键物理结果以供 Orchestrator 验收。
"""
...
def run_experiment(plan_file, experiment_dir, log_dir, model_orchestrator, model_coder, include_all_files=False, repo_url=None):
    ...
    while step_idx < len(plan_steps):
        ...
        while attempts < MAX_RETRIES and not step_passed:
            attempts += 1
            logger.info(f"\n--- [Step {step_idx + 1}] Orchestrator 主动权第 {attempts}/{MAX_RETRIES} 轮交互 ---")
            ...
            context_prompt += "【本轮你已执行的 Tool Call 及结果】\n"
            ...
            context_prompt += "请仔细阅读以上内容，并输出 JSON 指令。如确认当前步骤目标完成且结果正确（代码已完成编写，且能够正常运行，运行结果符合预期），调用 PASS_STEP。"
            context_prompt += """
            【交互格式】
            你的回复必须严格包含以下 JSON 结构（被 ```json 和 ```json 包裹）(注意：切勿漏掉```json!!!)：
            ```json
        ...
        # ====== 步进机制与 Git 线上回溯联动 ======
        if step_passed:
            git_commit_and_push(workspace_dir, step_idx + 1, repo_url)
            step_idx += 1
            tool_calls_history = []
            save_state(workspace_dir, step_idx, past_summaries, base_readme, tool_calls_history)
        else:
