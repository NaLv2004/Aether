...
def parse_dependencies(directory, py_files):
    ...
    for file in py_files:
        ...
        try:
            ...
            print(f"Warning: 无法解析 {file} 的语法结构, 错误: {e}")
            continue
...
def get_processing_order(py_files, dependencies):
    ...
    if len(order) != len(py_files):
        print("警告：检测到循环依赖！剩余文件将按照字母顺序强行处理。")
...
def main():
    ...
    print(f"[System] 初始化完成。工作目录: {WORKSPACE_DIR}")
    ...
    if not previous_summary:
        print(f"[警告] 未在工作目录找到 {PREVIOUS_SUMMARY_FILE}，或者文件为空。请确认。")
...
15.计划中的第3步必须探讨窄带模型下，S取不同值时，全连接MPNN和Sparse GNN的性能对比（第3步必须全连接MPNN和Spaese GNN都运行）。S至少取3个不同值，其中两个值必须大于77. 层数固定为4，特征维度固定为64.
16.计划的第4步必须为：
本步旨在对比宽带大容量GNN与宽带基线的性能。对比对象：充分训练的宽带Sparse GNN与宽带VR-MMSE的BER对比，仿真场景：首先运行sparse_gnn_wideband.py，参数直接定义为：N=256, K=8, F=16, S=128, f_c=0.1e12, B_bw=10e9, num_train=1000, num_val=2000, num_test=5000, epochs=100, batch_size=16, lr=0.001, hidden_dim=64, num_layers=4, snr_min=-5, snr_max=15, snr_list=-5 0 5 10 15, train_snr=10.0。
格式示例：
[
...
"""
    ...
    for step_item in plan:
        ...
        print(f"\n=============================================")
        print(f" 开始执行计划步骤 {step_idx}: {step_content}")
        print(f"=============================================")
...
"""
                ...
                if not eval_json:
                    print("[警告] 解析结果评估 JSON 失败，默认通过并保存。")
    ...
    print("\n[System] =========================================")
    print("[System] 所有实验计划已执行完毕，完整总结已存入:")
    print(f"[System] {EXECUTE_HISTORY_FILE}")
    print("[System] =========================================")
