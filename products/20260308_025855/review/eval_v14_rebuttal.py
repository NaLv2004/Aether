...
def parse_args():
    parser = argparse.ArgumentParser(description="Comprehensive Evaluation Script for V14 Rebuttal")
    ...
    parser.add_argument("--test_samples", type=int, default=1000, help="Total samples for evaluation smoothing")
...
def compute_mmse_pic_ber(y, H, sys_model, iterations=3):
    """Reviewer M5b: Classical MMSE-PIC Iterative Baseline."""
    B, L, N, K = H.shape
    H_flat = H.reshape(B, L*N, K)
    y_flat = y.reshape(B, L*N, 1)
    H_H = H_flat.conj().transpose(0, 2, 1)
    
    # Initial MMSE estimate
    R = H_H @ H_flat + sys_model.noise_w * np.eye(K)
    W = np.linalg.inv(R) @ H_H
    s_hat = (W @ y_flat).squeeze(-1)
    
    qpsk = np.array([1+1j, -1+1j, -1-1j, 1-1j]) / np.sqrt(2)
    
    for it in range(iterations - 1):
        dist = np.abs(s_hat[..., np.newaxis] - qpsk)
        s_tilde = qpsk[np.argmin(dist, axis=-1)] 
        s_hat_new = np.zeros_like(s_hat)
        
        for k in range(K):
            s_interf = s_tilde.copy()
            s_interf[:, k] = 0
            y_interf = H_flat @ s_interf[..., np.newaxis]
            y_k = y_flat - y_interf
            
            h_k = H_flat[:, :, k:k+1]
            h_k_H = h_k.conj().transpose(0, 2, 1)
            
            h_k_norm2 = np.sum(np.abs(h_k)**2, axis=1, keepdims=True)
            w_k = h_k_H / (h_k_norm2.transpose(0, 2, 1) + sys_model.noise_w)
            
            s_hat_new[:, k] = (w_k @ y_k).squeeze(-1).squeeze(-1)
            
        s_hat = s_hat_new
        
    return compute_dist_full_ber(s_hat)

def compute_cmmse_mixed_q_detection(y, H, sys_model, target_bits=8):
    """Reviewer M5a: SNR-proportional bit allocation for C-MMSE-MxQ."""
    B, L, N, K = H.shape
    ap_strength = np.sum(np.abs(H)**2, axis=(2,3)) 
    y_q = np.zeros_like(y)
    H_q = np.zeros_like(H)
    
    def q_array(x, b):
        if b <= 0: return np.zeros_like(x)
        if b >= 16: return x
    ...
    allowed = np.array([0, 4, 8, 12, 16])
    budget = L * target_bits
    
    for b_idx in range(B):
        strength = ap_strength[b_idx]
        weights = strength / (np.sum(strength) + 1e-10)
        continuous_bits = weights * budget
        
        bits_alloc = np.zeros(L, dtype=int)
        for l in range(L):
            idx = np.argmin(np.abs(allowed - continuous_bits[l]))
            bits_alloc[l] = allowed[idx]
            
        while np.sum(bits_alloc) > budget:
            valid_idx = np.where(bits_alloc > 0)[0]
            if len(valid_idx) == 0: break
            min_idx = valid_idx[np.argmin(strength[valid_idx])]
            bits_alloc[min_idx] -= 4
            
        while np.sum(bits_alloc) < budget:
            valid_idx = np.where(bits_alloc < 16)[0]
            if len(valid_idx) == 0: break
            max_idx = valid_idx[np.argmax(strength[valid_idx])]
            bits_alloc[max_idx] += 4

        for l in range(L):
            b_l = bits_alloc[l]
            y_q[b_idx, l] = q_array(y[b_idx, l], b_l)
            H_q[b_idx, l] = q_array(H[b_idx, l], b_l)
...
def run_model_eval(model, s_hat, H_est, y, local_snr, labels, device, dropout_rate=0.0, **kwargs):
    """Evaluates proposed model, natively supporting AP Dropout via dropout_rate mask."""
    B, L, K = s_hat.shape
    ...
    if dropout_rate > 0.0:
        mask = (torch.rand(B, L, device=device) > dropout_rate).float()
        v_t = v_t * mask.unsqueeze(-1).unsqueeze(-1)
        H_t = H_t * mask.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
        y_t = y_t * mask.unsqueeze(-1).unsqueeze(-1)
        snr_t = snr_t * mask.unsqueeze(-1).unsqueeze(-1)
...
def main():
    ...
    # Reviewer M8: Average across multiple large-scale fading seeds
    ...
    samples_per_seed = max(1, args.test_samples // len(seeds))

    # --- Task 1 & 2 ---
    for task_name, tau_val in [("Task 1: Perfect CSI", 0.0), ("Task 2: Imperfect CSI", 0.05)]:
        print(f"\n{'='*20} {task_name} (tau={tau_val}, Averaged over {len(seeds)} seeds) {'='*20}")
        if tau_val == 0.0:
            print(f"{'Ptx':>5} | {'Dist-Full':>10} | {'C-MMSE':>10} | {'MMSE-PIC':>10} | {'C-MMSE-MxQ':>10} | {'Prop BER':>10} | {'Prop Bits':>10}")
        else:
            print(f"{'Ptx':>5} | {'Dist-Full':>10} | {'C-MMSE':>10} | {'C-MMSE-MxQ':>10} | {'Prop BER':>10} | {'Prop Bits':>10}")
            
        for p in p_tx_list:
            res_dist, res_cmmse, res_pic, res_mxq, res_prop_ber, res_prop_bits = [], [], [], [], [], []
            ...
            for s in seeds:
                ...
                sh, s_true, H, snr, y, lbl = generate_eval_batch(sys_model, samples_per_seed, p, tau_val)
                
                res_dist.append(compute_ber(s_true, compute_dist_full_ber(sh)))
                res_cmmse.append(compute_ber(s_true, compute_cmmse_ber(y, H, sys_model)))
                res_mxq.append(compute_ber(s_true, compute_cmmse_mixed_q_detection(y, H, sys_model, target_bits=8)))
                
                if tau_val == 0.0:
                    res_pic.append(compute_ber(s_true, compute_mmse_pic_ber(y, H, sys_model)))
                
                ber_p, bits_p = run_model_eval(model, sh, H, y, snr, lbl, device)
                res_prop_ber.append(ber_p)
                res_prop_bits.append(bits_p)
                
            if tau_val == 0.0:
                print(f"{p:5} | {np.mean(res_dist):10.5f} | {np.mean(res_cmmse):10.5f} | {np.mean(res_pic):10.5f} | {np.mean(res_mxq):10.5f} | {np.mean(res_prop_ber):10.5f} | {np.mean(res_prop_bits):10.1f}")
            else:
                print(f"{p:5} | {np.mean(res_dist):10.5f} | {np.mean(res_cmmse):10.5f} | {np.mean(res_mxq):10.5f} | {np.mean(res_prop_ber):10.5f} | {np.mean(res_prop_bits):10.1f}")

    # --- Task 3: Generalization ---
    print(f"\n{'='*20} Task 3: Generalization to K=12 (tau=0.05, Averaged over {len(seeds)} seeds) {'='*20}")
    ...
    for p in p_tx_list:
        ...
        for s in seeds:
            ...
            sh, s_true, H, snr, y, lbl = generate_eval_batch(sys_k12, samples_per_seed, p, 0.05, K_override=12)
            
            res_dist.append(compute_ber(s_true, compute_dist_full_ber(sh)))
            ber_p, _ = run_model_eval(model, sh, H, y, snr, lbl, device)
            res_prop.append(ber_p)
            
        print(f"{p:5} | {np.mean(res_dist):15.5f} | {np.mean(res_prop):15.5f}")

    # --- Task 4: Pareto Front ---
    print(f"\n{'='*20} Task 4: Pareto Front (Ptx=10dBm, tau=0.05, Averaged over {len(seeds)} seeds) {'='*20}")
    p_pareto = 10
    penalties = [0.0, 0.02, 0.05, 0.1, 0.2]
    print(f"{'Penalty':>8} | {'Avg Bits':>10} | {'Prop BER':>10} | {'Baseline (Q-Dist) BER'}")
    
    res_pareto = {bp: {'bits': [], 'prop': [], 'base': []} for bp in penalties}
    for s in seeds:
        np.random.seed(s); torch.manual_seed(s)
        sys_model = CellFreeSystem(args.L, args.N, args.K, args.area_size, args.bandwidth, args.noise_psd, 0.0)
        sys_model.generate_scenario()
        sh, s_true, H, snr, y, lbl = generate_eval_batch(sys_model, samples_per_seed, p_pareto, 0.05)
        for bp in penalties:
            ...
            res_pareto[bp]['bits'].append(bits_p)
            res_pareto[bp]['prop'].append(ber_p)
            res_pareto[bp]['base'].append(ber_base)
            
    for bp in penalties:
        print(f"{bp:8.2f} | {np.mean(res_pareto[bp]['bits']):10.1f} | {np.mean(res_pareto[bp]['prop']):10.5f} | {np.mean(res_pareto[bp]['base']):10.5f}")

    # --- Task 5: Ablation ---
    print(f"\n{'='*20} Task 5: Ablation Study (tau=0.05, Averaged over {len(seeds)} seeds) {'='*20}")
    ab_p_tx = [-10, 0, 10, 20]
    modes = [
        ("Full Proposed", {}),
    ...
    print(header)
    for p in ab_p_tx:
        res_abl = {name: [] for name, _ in modes}
        for s in seeds:
            np.random.seed(s); torch.manual_seed(s)
            sys_model = CellFreeSystem(args.L, args.N, args.K, args.area_size, args.bandwidth, args.noise_psd, 0.0)
            sys_model.generate_scenario()
            sh, s_true, H, snr, y, lbl = generate_eval_batch(sys_model, samples_per_seed, p, 0.05)
            
            for name, m_kwargs in modes:
                ber_p, _ = run_model_eval(model, sh, H, y, snr, lbl, device, **m_kwargs)
                res_abl[name].append(ber_p)
                
        row = f"{p:<6}"
        for name, _ in modes:
            row += f"| {np.mean(res_abl[name]):15.5f} "
        print(row)

    # --- Task 6: AP Dropout Robustness ---
    # Reviewer M6: Robustness under node failures
    print(f"\n{'='*20} Task 6: AP Dropout Robustness (Ptx=10dBm, tau=0.05, Averaged over {len(seeds)} seeds) {'='*20}")
    dropout_rates = [0.0, 0.1, 0.3, 0.5]
    print(f"{'Dropout Rate':>12} | {'Proposed BER':>12} | {'Degradation':>12}")
    
    res_drop = {dr: [] for dr in dropout_rates}
    for s in seeds:
        np.random.seed(s); torch.manual_seed(s)
        sys_model = CellFreeSystem(args.L, args.N, args.K, args.area_size, args.bandwidth, args.noise_psd, 0.0)
        sys_model.generate_scenario()
        sh, s_true, H, snr, y, lbl = generate_eval_batch(sys_model, samples_per_seed, 10, 0.05)
        
        for dr in dropout_rates:
            ber_p, _ = run_model_eval(model, sh, H, y, snr, lbl, device, dropout_rate=dr)
            res_drop[dr].append(ber_p)
            
    base_ber = np.mean(res_drop[0.0])
    for dr in dropout_rates:
        ber = np.mean(res_drop[dr])
        deg = ber - base_ber
        print(f"{dr:12.1f} | {ber:12.5f} | {deg:12.5f}")
