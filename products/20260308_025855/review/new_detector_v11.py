...
def parse_args():
    parser = argparse.ArgumentParser(description="GNN+Transformer Hybrid Detector V11 (MRC-Enhanced, Residual Logits, Triple Quant V9 Logic)")
    # System parameters
...
# ============================================================
def generate_data_batch_v11(sys_model, batch_size, p_tx_dbm, tau_est=0.0):
...
# ============================================================
# 2. Triple Policy & Quantizer (V9 Logic)
# ============================================================
class TriplePolicyNetworkV11(nn.Module):
...
class TripleAdaptiveQuantizerV11(nn.Module):
    def __init__(self, N, policy_hidden=64):
        super().__init__()
        self.N = N
        self.policy = TriplePolicyNetworkV11(input_dim=9, policy_hidden=policy_hidden)
    ...
    def forward(self, v, H, y, local_snr, tau=1.0):
        ...
        H_q = apply_q(H, w_H, self.qs_H, [B, L, K, 1, 1])
...
# ============================================================
# 3. Detector (V10 Mechanisms: MRC + Residual Logits)
# ============================================================
...
class GNNTransformerDetectorV11(nn.Module):
...
class JointModelV11(nn.Module):
    def __init__(self, L, N, K, hidden_dim=128):
        super().__init__()
        self.quantizer = TripleAdaptiveQuantizerV11(N)
        self.detector = GNNTransformerDetectorV11(L, N, K, hidden_dim)
...
def main():
    ...
    for p in test_p_tx:
        sh, s, H, snr, y, lbl = generate_data_batch_v11(sys_model, args.test_samples, p, args.tau_est)
        test_set[p] = {
    ...
    model = JointModelV11(args.L, args.N, args.K, args.hidden_dim).to(device)
    ...
    for epoch in range(args.epochs_phase1):
        ...
        for _ in range(args.batches_per_epoch):
            sh, s, H, snr, y, lbl = generate_data_batch_v11(sys_model, args.batch_size, np.random.uniform(5, 15), args.tau_est)
    ...
    torch.save(model.state_dict(), "new_joint_model_v11.pth")
