...
def parse_args():
    ...
    # Pareto 敏感度分析相关参数
    parser.add_argument("--lambda_val", type=float, default=0.1, help="Weight for bit loss constraint")
    parser.add_argument("--c_avg", type=float, default=2.5, help="Target average bits per AP-user link")
...
def main():
    ...
    print(f"Hyperparameters: Epochs={args.epochs}, Batches={args.batches_per_epoch}, BatchSize={args.batch_size}, LR={args.lr}, Lambda={args.lambda_val}, C_avg={args.c_avg}")
    ...
    for epoch in range(args.epochs):
        ...
        for _ in range(args.batches_per_epoch):
            ...
            # Loss计算: 最终复预测的 MSE误差 + 确保总体系统平均吞吐在 args.c_avg 左右的约束惩罚
            mse_loss = mse_loss_fn(out, Y_train)
            bit_loss = args.lambda_val * ((expected_bits.mean() - args.c_avg) ** 2)
    ...
    # 保存模型权重
    save_path = f'qat_gnn_model_c{args.c_avg}_l{args.lambda_val}.pth'
    torch.save(model.state_dict(), save_path)
    print(f"\nJoint QAT Model weights have been saved to '{save_path}'.")
