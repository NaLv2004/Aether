...
def parse_args():
    parser = argparse.ArgumentParser(description="GNN+Transformer Hybrid Detector V13 (MRC-Enhanced, Residual Logits, Triple Quant V13 Logic)")
    # System parameters
    ...
    parser.add_argument("--hidden_dim", type=int, default=256, help="Hidden dimension D for feature embedding")
    ...
    parser.add_argument("--num_transformer_layers", type=int, default=3, help="Number of Transformer encoder layers")
...
# ============================================================
def generate_data_batch_v13(sys_model, batch_size, p_tx_dbm, tau_est=0.0):
...
# ============================================================
# 2. Triple Policy & Quantizer (V13 Logic)
# ============================================================
class TriplePolicyNetworkV13(nn.Module):
...
class TripleAdaptiveQuantizerV13(nn.Module):
    def __init__(self, N, policy_hidden=64):
        super().__init__()
        self.N = N
        self.policy = TriplePolicyNetworkV13(input_dim=9, policy_hidden=policy_hidden)
...
# ============================================================
# 3. Detector (V13 Mechanisms: Normalized MRC + Adjusted Residual Logits)
# ============================================================
...
class GNNTransformerDetectorV13(nn.Module):
    def __init__(self, L, N, K, hidden_dim=256, num_gnn_layers=3, num_transformer_layers=3, num_heads=4, dropout=0.1):
        ...
        # Initializing parameters to 1.0 to encourage Transformer learning
...
class JointModelV13(nn.Module):
    def __init__(self, L, N, K, hidden_dim=256):
        super().__init__()
        self.quantizer = TripleAdaptiveQuantizerV13(N)
        self.detector = GNNTransformerDetectorV13(L, N, K, hidden_dim)
...
def main():
    ...
    for p in test_p_tx:
        sh, s, H, snr, y, lbl = generate_data_batch_v13(sys_model, args.test_samples, p, args.tau_est)
        test_set[p] = {
    ...
    model = JointModelV13(args.L, args.N, args.K, args.hidden_dim).to(device)
    ...
    for epoch in range(args.epochs_phase1):
        ...
        for _ in range(args.batches_per_epoch):
            sh, s, H, snr, y, lbl = generate_data_batch_v13(sys_model, args.batch_size, np.random.uniform(5, 15), args.tau_est)
    ...
    for epoch in range(args.epochs_phase2):
        ...
        for _ in range(args.batches_per_epoch):
            ...
            opt2.zero_grad()
            logits, eb = model(v_t, H_t, y_t, snr_t, tau=tau, use_quantization=True)
    ...
    torch.save(model.state_dict(), "new_joint_model_v13.pth")
